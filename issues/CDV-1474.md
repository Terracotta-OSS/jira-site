...
map.put("key1", null) does lead to an ObjectID.NULL\1ID
in the references map in PartialMapManagedObjectState.

We don't filter Object.NULL\1ID these at the managed object state class
...
2. See if it acceptable for YoungGen to have NULL\1ID's
...
Ran a test maps with null values test and found out that when you put a null, the value is an ObjectID.NULL\1ID.
...
The only way this can fail is if the rootId happens to be an Object.NULL\1ID, if you notice in YoungGenChangeCollectorImpl.notifyObjectEvicted.
If a logical object happens to make it into the rememberedSet where its children does have Object.NULL\1ID, then this can happen,
...
Is this bug fixed in TC 3.2.1\12?
If not - 
...
This assertion shouldn't happen in TC\13.2.1\12. You should be ok with that revision.
